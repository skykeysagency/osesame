...
<form class="cart grouped_form" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
	<table class="woocommerce-grouped-product-list group_table">
		...
		foreach ( $grouped_products as $grouped_product_child ) {
			...
			echo ( '<td>' );
			echo wp_kses(
				$grouped_product_child->get_image(
					[ 640, 640 ],
					[
						'class' => 'max-w-full w-full',
						'alt'   => esc_attr($grouped_product_child->get_name()),
					]
				),
				'rbb-kses'
			);
				...
				switch ( $column_id ) {
					case 'quantity':
						...
						if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
							...
							echo '<label for="' . esc_attr('quantity-' . $grouped_product_child->get_id()) . '" class="screen-reader-text">' . esc_html__('Buy one of this item', 'botanica') . '</label>';
						} else {
							...
							woocommerce_quantity_input(
								[
									...
									'input_value' => isset($_POST['quantity'][ $grouped_product_child->get_id() ]) ? wc_stock_amount(wc_clean(wp_unslash($_POST['quantity'][ $grouped_product_child->get_id() ]))) : '', // phpcs:ignore
						...
						break;
					case 'title':
						$value  = '<label class="block font-semibold" for="product-' . esc_attr($grouped_product_child->get_id()) . '">';
						...
						break;
					case 'label':
						$value = wc_get_stock_html($grouped_product_child);
						break;
					default:
						$value = '';
						break;
