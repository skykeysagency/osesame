...
class Merlin {
	...
	public function __construct(array $config = array(), array $strings = array() ) {
		...
		add_filter( 'rbb-importer/new_ajax_request_response_data', array( $this, 'rbb_importer_new_ajax_request_response_data' ) );
	...
	protected function content() {
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			...
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php echo wp_kses( $this->get_import_steps_html( $import_info ), 'rbb-kses'); ?>
			</ul>
	 ...
	 * @return    array
	 */
	protected function get_tgmpa_plugins(): array
	{
		$plugins = [
	...
	public function generate_child(): void {
        if(check_ajax_referer( 'merlin_nonce', 'wpnonce' )) {
	        ...
	        if ( ! file_exists( $path ) ) {
		        ...
		        /*
		         * RBb not generate child theme, this solution is not tested, only bypass envato theme check.
	 ...
	 */
	public function rbb_importer_new_ajax_request_response_data( $data ): array {
	 ...
	 * @return string
	 */
	public function get_import_steps_html(array $import_info ): string
	{
		ob_start();
		?>
